  ...
  server.registerTool(
    'bridge_send_message',
    {
      ...
      description:
        ...
        + '**Recipient relay rule**: when YOU receive an inbound bridge message via the `<channel source="agent-bridge">` block, relay a compact 1-3 sentence summary (sender machine + target + actionable ask) to the user via your harness\'s configured user-facing channel (Telegram, Slack, Discord, native UI, etc.) so the user has live visibility into cross-harness coordination without dumping the full bridge body. **Append the running agent-bridge version at the end** of the relay (e.g. `_(agent-bridge v4.1.0)_`) — read it from the BRIDGE-CONTEXT block\'s `agent_bridge_version` field (OpenClaw) or from `claude_code_channel_status` (Claude Code); do NOT hardcode it. If the user asks to expand an OpenClaw `[Agent Bridge relay]` notice by its `expand id`, run `agent-bridge relay-expand <id>` on that same machine and send the retrieved full content subject to normal privacy/channel rules. Reply via bridge first if needed, THEN relay to the user. Don\'t suppress routine messages except pure-noise heartbeats / `bridge_status` polls. See AGENTS.md "Relay inbound bridge messages to the user" + canonical doc docs/relay-to-user.md.\n\n'
