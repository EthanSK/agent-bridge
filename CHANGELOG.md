...
- **New package `codex-channel/`** (plain ESM, zero deps, macOS+Windows paths): binding registry (`~/.agent-bridge/codex/bindings.json`, atomic writes, corrupt-file preservation), single-daemon service with lease/heartbeat/fenced takeover (`locks/codex-channel.lock.json`), per-alias inbox watcher over `inbox/codex/<alias>/`, App Server stdio JSON-RPC client with initialize/version negotiation + bounded request timers + fragmentation-safe framing, and a crash-safe delivery state machine (atomic claim → `staged`/`turn_starting`/`turn_active` meta sidecars, non-expiring delivered/indeterminate id tombstones, strict FIFO per alias, 5s/30s/120s backoff then dead-letter). Ambiguous sends use a structural history probe that prefers paginated `thread/items/list` and falls back to exact-hit-only `thread/read(includeTurns)` when the runtime reports that method unsupported; anything not proven delivered is preserved as an explicit indeterminate dead letter and is never automatically re-sent.
- **Existing-task opt-in**: `agent-bridge codex bind` run inside the task shell auto-discovers `CODEX_THREAD_ID` and a stable title-derived alias (`--alias <name>` overrides it); `--thread-id` for explicit binds; rollout-existence validation with `--force` escape; duplicate/rebind/shared-thread rules; `list|status|doctor|enable|disable|unbind|rebind` lifecycle without file editing. Same flow via MCP: `bridge_codex_bind` / `bridge_codex_unbind` / `bridge_codex_status`.
- **Delivery composition** preserves the established conventions: `<channel source="agent-bridge" …>` wrapper + `[BRIDGE-CONTEXT]` metadata (source/destination machines+targets, exact thread id, message id, reply-to, both agent-bridge versions, attempt, policy, timestamps, reply expectation, exact reply tool call). The remote body is boundary-labeled and neutralized against fence/context/channel forgery; wire metadata is control-character-stripped. This is structural integrity, not a prompt-authority sandbox.
- **Safety and trust boundary**: bridge-delivered turns NEVER auto-approve — command/file/permission approvals are declined, permissions grant nothing (turn scope), and user-input/elicitation requests get schema-valid empty/decline responses. Delivered-turn overrides use `approvalPolicy: "never"` and sandbox `workspaceWrite` scoped to the canonical binding/effective task cwd with network off and both implicit `$TMPDIR`/`/tmp` writable roots excluded; because Codex persists those values, the service durably journals and restores the pre-bridge thread settings before release. `danger-full-access` is CLI-only and requires fresh `--allow-danger` consent on every effective-danger bind/rebind; MCP refuses explicit or inherited danger. A paired sender targeting a bound alias nevertheless supplies a user prompt and can ask the model to use tools already exposed to the task; Codex 0.145 exposes no per-turn tool allowlist/isolation, so binding grants prompt/tool authority to that peer. The ownership guard defers delivery while another Codex client appears to be writing the thread and surfaces explicit `thread_not_found` / `ownership_conflict` / `codex_unsupported` states instead of substituting threads; Codex locks remain process-local, so this is a race-reduction policy rather than a cross-process single-writer guarantee.
- **Outbound identity for tools-only hosts**: new `AGENT_BRIDGE_FROM_TARGET` env (set in the Codex MCP entry) makes `bridge_send_message` default `from_target` to the bound `codex/<alias>`; bound Claude personas always win, invalid values fall back to the legacy `claude-code/default` — zero behavior change for existing Claude/OpenClaw installs. Plus a no-MCP fallback: `agent-bridge codex send` delivers over the same SFTP/local transport as every other bridge send.
- **Lifecycle**: `agent-bridge codex …` dispatcher in the root CLI; MCP server boot ensures the daemon when enabled bindings or pending-settings recovery journals exist (tools-only mode included; kill switch `AGENT_BRIDGE_CODEX_NO_ENSURE=1`); periodic updaters (macOS + Windows) use the same recovery-work gate. Automatic version replacement drains active bridge turns while keeping the old lease heartbeat alive and blocks new turn dispatch until safe restore/unsubscribe/teardown; an explicit operator stop remains abortive and preserves durable settings recovery state.
- **Observability**: unified-log `codex.*` events across binding, ownership, connection, delivery states, retries, quarantine, reply expectation, and shutdown; human `status`/`doctor` output including unbound-alias inboxes and dead-letter counts.
- **Tests**: 285 deterministic `node:test` cases across codex-channel (284 runnable on macOS plus one Windows-junction skip): protocol/version negotiation, framing, exact resume, active-turn hydration, automatic alias selection/reuse/collisions, early lifecycle races, delivery with `clientUserMessageId=BridgeMessage.id`, queueing/guarded steering, schema-exact declines, atomic claims/tombstones/replay/paginated dedupe/quarantine/consume races, lease/dispatch/PID/process-tree safety, registry concurrency and stale-recovery ABA stress, service startup readiness/version drain/reload/idle-close/shutdown, durable settings recovery, ownership guard, isolated installer/package flows, status/doctor, CLI subprocess flows, and vendor/version parity; plus 206 unified MCP regression cases (204 runnable on macOS and two platform-only skips).
- **Version bumps.** CLI / MCP server / Claude plugin metadata / OpenClaw channel / codex-channel all move to `4.10.0` in lockstep. No BridgeMessage wire-format changes; Claude Code and OpenClaw routing, inbox layout, and leases untouched.
- **Distribution truth.** 4.10.0 is released through GitHub source archives, the one-line installers, the Claude Code local marketplace, and the repo-loaded OpenClaw plugin. The registry package `agent-bridge-mcp-server` remains a legacy 0.1.x artifact and is not a supported current install path; the scoped OpenClaw/Codex packages are self-contained repository units rather than published npm products.

**Release-hardening pass (same 4.10.0, pre-release)** — independent Codex/protocol/packaging reviews found blockers in the first implementation; all fixed with regression fences:

- **Exact Codex 0.145 protocol**: the dedupe probe prefers `thread/items/list` with exact generated-schema pagination (`threadId`/`cursor`/`limit`/`sortDirection: "desc"`), matching only the structural `userMessage.clientId` to the bridge message id and iterating to `nextCursor: null` with cursor-loop detection and bounded page/item caps. Installed Codex 0.145 currently can return `-32601 not supported yet` for that method, so `thread/read includeTurns` is a load-bearing structurally hit-confirming fallback; a miss remains INDETERMINATE because 0.145 rejects that hydration for paginated histories. `thread/resume` requests a recent full page (`initialTurnsPage {limit, sortDirection: "desc", itemsView: "full"}`), hydrates in-progress turns from BOTH legacy `thread.turns` and `initialTurnsPage.data` as foreign/unowned before declaring idle, and an `active` thread status with no recoverable turn id enters a safe queue-only busy state cleared by the next completion; a resume without a verifiable returned thread id is a failure; approvals answer the exact `{decision:"decline"}` shape (no extra fields); `item/tool/requestUserInput` answers every question with `{answers:{<id>:{answers:[]}}}`; `mcpServer/elicitation/request` declines with `{action:"decline",content:null,_meta:null}`; unsupported `on-failure` approval policy removed (supported subset: never/on-request/untrusted; stored legacy values migrate, inputs are rejected); granular AskForApproval OBJECTS are captured and restored verbatim; the identity-dropping `clientUserMessageId` retry was removed (durable identity is a reliability invariant on the 0.145 floor); pending-start lifecycle correlation attributes an early `turn/completed` (arriving before the `turn/start` response) to OUR turn exactly once instead of losing ownership.
- **Durable delivery state + AT-MOST-ONCE**: meta sidecars are written atomically (temp+fsync+rename+dir-fsync-where-supported); the indeterminate `turn_starting` marker must be durably on disk BEFORE any `turn/start`/`turn/steer` leaves the machine (write failure = no send); claimed files with missing/corrupt metadata reconcile as INDETERMINATE while the normal claim path stays probe-free; finalize records a durable `delivered` state before archive side effects so crashes complete the finalize instead of reopening delivery. Non-expiring hashed delivered tombstones and separate possibly-sent terminal tombstones survive restart and ledger rollover; retained state is validated fail-closed before service startup. Once an ambiguous send exists (timeout/dropped pipe/lost metadata), probe ABSENCE is NOT permission to resend — Codex may still accept the original later and documents no idempotent duplicate suppression — so delivery probes through a bounded late-acceptance backoff window and then dead-letters explicitly (`indeterminate_ambiguous_send_absent_no_resend` / `indeterminate_delivery_probe_unavailable`) for manual recovery, with NO second turn ever; clean pre-acceptance RPC failures still retry normally.
- **Registry concurrency**: all binding mutations run under an interprocess lock whose fully written/fsynced unique owner record is atomically hard-linked into place (ownership token + bounded stale recovery that never steals a young/live lock); reload-inside-lock ends the parallel-bind record loss (24-way cross-process regression test), and the bounded acquisition window accommodates serialized durable fsync work under host load; existing-but-unreadable registries fail mutations closed; corrupt bytes are only quarantined-aside under the lock, never by read-only status calls.
- **Session/policy correctness**: rebinding an alias — including same-thread rebinds — invalidates the live session whenever cwd/sandbox/approvals/steering change (danger-full-access can no longer outlive a rebind to read-only); per-thread multi-session request dispatch with connection-level fail-closed declines (releasing one alias never orphans another's approvals); steering only ever targets bridge-owned turns; terminal turn outcomes (`codex.turn_completed`/`codex.turn_failed_terminal`) are logged, message-attributed, and surfaced in status; pre-bridge thread settings (cwd/approval/sandbox — which Codex persists across turns) are captured at resume and restored via `thread/settings/update` after bridge turns, with guarded skip when a foreign turn intervened; ownership guard now also detects foreign rollout writes AFTER attach and releases; sessions release promptly when the queue drains instead of holding a second writer for the idle window.
- **Lease/PID/process-tree safety**: daemon heartbeat runs on an independent timer (slow 20-30 s RPCs cannot stale a live lease); lease mutation and every turn dispatch are token-fenced; stop/refresh use token-bound control IPC with LIVE-OWNERSHIP PROOF and never signal a raw lease PID, so PID reuse cannot kill an unrelated process. App Server replacement requires verified teardown of the exact Unix process group or Windows `/PID ... /T` tree; uncertainty persists a poison fence. Detached startup is reported healthy only after the exact winning lease token advances its heartbeat, and concurrent ensure callers reconcile to that one proven winner.
- **Self-contained packaging**: codex-channel vendors its shared openclaw-channel modules (byte-parity-tested + `scripts/sync-codex-vendor.mjs`); mcp-server vendors the shared `lib/` relay modules the same way (npm `prebuild` sync), and Codex support loads LAZILY with candidate resolution (`AGENT_BRIDGE_SOURCE_DIR` honored first, then repo layout, then the installed `<resolved AGENT_BRIDGE_HOME>/runtime/codex-channel/`) — the exact marketplace/cache artifact now starts, registers every tool, and degrades codex handlers with actionable errors when the runtime is absent (isolated-artifact tests prove it end-to-end over real MCP stdio); install.sh/install.ps1 normalize either accepted `AGENT_BRIDGE_HOME` form, stage-swap the self-contained runtime snapshot, and version-ensure from the newly installed copy when recovery work exists, so the one-line install can run `agent-bridge codex …` without a checkout.
- **Queues + hostile input**: strict FIFO per alias (a backoff-parked head is never bypassed; bounded by dead-letter caps); backoff schedule fixed so every documented delay (5s/30s/120s) is reachable before the 4th-failure dead-letter; pre-parse file guards (lstat regular-file check — symlinks quarantined unread — and a total-size cap), finite sane-range timestamps, bounded metadata strings, and bounded composition-failure handling.
- **Updater/lifecycle**: periodic updaters and their provisioners normalize and preserve either accepted `AGENT_BRIDGE_HOME` form; ensure the daemon when at least one ENABLED binding or a pending-settings recovery journal exists; honor `AGENT_BRIDGE_CODEX_NO_ENSURE=1`; and on PowerShell check `$LASTEXITCODE` for native commands. A manual `service stop` is temporary while a supervisor still sees recovery work.
- **Discoverability/docs**: `bridge_send_message` description/schema now name `codex/<alias>` and the `AGENT_BRIDGE_FROM_TARGET` precedence; version parity extended to package-lock + the `MCP_SERVER_VERSION` constant; stale “Codex is scaffolded/unverified” claims replaced repo-wide with the precise model (the task never polls; a background service scans the inbox and wakes the exact persisted task via the App Server — not a native harness channel push).
...
- **Store.** `~/.agent-bridge/shared-context/learnings.ndjson` — append-only NDJSON, full replica per machine, one entry per line: `id` (lowercase uuid, the dedupe key — mirrors the bridge-message id convention), `ts`, `machine`, `harness`, `title`, `body`, `tags[]`, `scope: "global"`, `v: 1`. Greppable/jq-able by hand; `learnings list/show/search` render the human view.
- **CLI verbs.** `agent-bridge learnings add|search|list|show|sync|remove|ingest`. `add` appends locally then best-effort pushes to every paired peer by SSHing the PEER's own `agent-bridge learnings ingest --json '<entry>'` (the remote owns its store writes + dedupe — same side-effect-path architecture as `notify`; a replica write is NOT an agent message, so it never rides the inbox/watcher path). Offline peers reconcile bidirectionally via `learnings sync [machine|--all]` (batch push over stdin, one SSH hop). `remove <id>` is local-only by design (no tombstones; a later sync can resurrect from a peer); `remove <id> --fleet` deletes on every paired machine. Everything is idempotent by entry id, so pushes/syncs/replays are harmless.
- **MCP tools.** `bridge_learnings_add` (record + replicate, per-peer success/failure reported) and `bridge_learnings_search` (pure local read — every machine holds a full replica, so searching never needs the network). Server instructions now tell every connected agent: you CAN search the shared context when debugging (another agent may have already solved it) and MUST record any learning that could apply fleet-wide — with scope discipline (project-local fixes stay in that repo's `LEARNINGS.md`; harness-private notes stay in harness memory).
- **Mirrored implementation pair.** Store semantics live in `mcp-server/src/learnings.ts` (TS, used by the MCP tools locally) and the `cmd_learnings` bash verbs (used by the CLI and by every remote ingest) — same contract as the notify renderer pair; keep them behaviorally identical.
- **Tests.** `mcp-server/test/learnings-store.test.mjs` (entry creation, dedupe-by-id idempotency, corrupt-line resilience, search semantics, and a REAL-shell round-trip of a hostile-body entry through `buildRemoteIngestCommand` against a stubbed remote CLI) + `test/cli-learnings.sh` (add/search/list/show/ingest/remove, uppercase-id normalization, batch ingest counts, push-on-write over stubbed ssh). Fixed a real bug the tests caught pre-ship: bash `add` wasn't deduping tags (TS was).
- **Docs.** README "Shared context" feature section, INSTRUCTIONS.md + AGENTS.md sections, both bundled skills (`skills/bridge`, `skills/openclaw`), site feature card, MCP server instructions block.
- **Version bumps.** CLI / MCP server / Claude plugin metadata all moved to `4.9.0` in lockstep (plugin descriptions now list 10 tools). No BridgeMessage wire-format changes.
